...
print.MonoClust <- function(x, abbrev = 0, spaces = 2, digits = options("digits")$digits, ...) {
    ...
    ## REMOVE: Tan, 12/14. This line seems to be a debug line. Mo meaning.  print(yval) ADD: Tan,
    ## 3/1/15. Add p value check.
    has.pvalue <- ifelse(is.null(frame$p.value), FALSE, TRUE)
    term <- rep(" ", length(depth))
    term[frame$var == "<leaf>"] <- "*"
    z <- labels(x, digits = digits, abbrev = abbrev, minlength = minlength, ...)
    n <- frame$n
    ## MODIFY: Tan, 3/1/15. Add p value.  z <- paste(indent, z, n, format(signif(frame$dev, digits =
    ## digits)), round((1-as.numeric(yval)/1),digits=2), term)
    if (has.pvalue) {
        z <- paste(indent, z, n, format(signif(frame$dev, digits = digits)), round((1 - as.numeric(yval)/1), 

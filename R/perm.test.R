...
perm.test <- function(object, data, auto.pick = FALSE, sig.val = 0.05, method = 1, rep = 10000, 
    ...
    # if(getRversion() >= '2.15.1') utils::globalVariables(c('.Jump_Table', '.Data'), add = FALSE)
    ...
    # Now tracing the tree to find the cluster Trace along the split order, quit when reaching the
    ...
    for (i in 1:max(na.omit(jump.table$split.order))) {
        ...
        jump.table$members[jump.table$left[current]] <- paste(members.L, collapse = ",")
        
        members.R <- setdiff(members, members.L)
        jump.table$members[jump.table$right[current]] <- paste(members.R, collapse = ",")
        
        p.value.unadj <- test.split(current, members, members.L, members.R, auto.pick, method, data, 
            split.var, jump.table$node[current], rep, stat)
    ...
    # # Tracing the tree by Node-Left-Right algorithm assign('.Jump_Table', jump.table, envir =
    # .GlobalEnv) # assign('.Data', data, envir = .GlobalEnv) recursive.walk(1, 1:nrow(data),
    # auto.pick, sig.val, method, fulldata = data) jump.table <- .Jump_Table rm(list =
...
test.split <- function(current, members, members.L, members.R, auto.pick, method, fulldata, split.var, 
    node, rep, stat) {
    REP <- rep
    
    # Membership is consecutive because the distance matrix will be moved around with members.L and
    # members.R put next to each other.
    ...
    if (method == 1) {
        ...
        # Method 2, 3: shuffling the splitting variable split again on that variable (method 2) or split
        ...
        for (k in 1:permutations) {
            ...
            if (method == 2) {
                cluster.rep <- MonoClust(toclust = currentdata, nclusters = 2, variables = split.var)
            } else if (method == 3) {
                cluster.rep <- MonoClust(toclust = currentdata, nclusters = 2)
            }
            ...
            if (stat == "F") {
                # If no split is made because of minbucket, cluster membership will have 1 in it. In that case,
                # F-stat = 0
                stat.rep[k] <- ifelse(1 %in% fmem2.rep, 0, F.stat(dist.mat.rep ~ fmem2.rep))
            } else if (stat == "AW") {
                stat.rep[k] <- ifelse(1 %in% fmem2.rep, 0, fpc::cluster.stats(dist.mat.rep, fmem2.rep)$avg.silwidth)
            }
        }
...
# recursive.walk <- function(current, members, auto.pick, sig.val, method, fulldata) { REP <-
# 100 # Check stopping at leaf node if (is.na(.Jump_Table$bipartvar[current])) return(0) # Node
# split.var <- .Jump_Table$bipartvar[current] split.value <- .Jump_Table$cut[current] # Take
# observations in that branch out and split into Left and Right members.L <-
# members[which(fulldata[members, split.var] < split.value)] members.R <- setdiff(members,
# members.L) # Membership is consecutive because the distance matrix will be moved around # with
# members.L and members.R put next to each other.  fmem2 <- factor(c(rep(1, length(members.L)),
# rep(2, length(members.R)))) if (method == 1) { # Method 1: shuffling the membership data.temp
# <- fulldata data.temp[,split.var] <- NULL distmat.reduced <-
# as.matrix(daisy(data.frame(data.temp))) # A distance matrix with observations left and right
# put in order dist.mat.twogroup <- distmat.reduced[c(members.L, members.R),c(members.L,
# members.R)] result <- adonis(dist.mat.twogroup ~ fmem2) pvalue.adj <-
# (as.numeric(row.names(.Jump_Table[current,])) %/% 2 + 1) * result$aov.tab[1,6] } else if
# (method == 2) { # Method 2: shuffling the splitting variable, split again on that variable
# currentdata <- fulldata[c(members.L, members.R), ] dist.mat.twogroup <-
# as.matrix(daisy(data.frame(currentdata))) f.stat.obs <- F.stat(dist.mat.twogroup ~ fmem2)
# f.stat.rep.c <- numeric(REP) # Shuffling for (k in 1:REP) { shuffled.splitting.var <-
# sample(currentdata[,split.var], size = nrow(currentdata), replace = FALSE)
# currentdata[,split.var] <- shuffled.splitting.var cluster.rep.constrained <- MonoClust(toclust
# = currentdata, nclusters = 2, variables = split.var) dist.mat.rep.c <-
# cluster.rep.constrained$Dist fmem2.rep.c <- cluster.rep.constrained$Membership f.stat.rep.c[k]
# <- F.stat(dist.mat.rep.c ~ fmem2.rep.c) } pvalue.adj <- sum(f.stat.rep.c < f.stat.obs) / REP }
# else if (method == 3) { # Method 3: shuffling the splitting variable, clustering on all
# variables dist.mat.twogroup <- as.matrix(daisy(data.frame(data.temp))) f.stat.obs <-
# F.stat(dist.mat.twogroup ~ fmem2) f.stat.rep.u <- numeric(REP) # Shuffling for (k in 1:REP) {
# shuffled.splitting.var <- sample(data.temp[,split.var], size = nrow(data.temp), replace =
# FALSE) data.temp[,split.var] <- shuffled.splitting.var cluster.rep.unconstrained <-
# MonoClust(toclust = data.temp, nclusters = 2) dist.mat.rep.u <- cluster.rep.unconstrained$Dist
# fmem2.rep.u <- cluster.rep.unconstrained$Membership f.stat.rep.u[k] <- F.stat(dist.mat.rep.u ~
# fmem2.rep.u) } pvalue.adj <- sum(f.stat.rep.u < f.stat.obs) / REP }
# .Jump_Table$p.value[current] <<- ifelse(pvalue.adj > 1, 1, pvalue.adj) if (auto.pick &&
# (pvalue.adj > sig.val)) return(0) # Left recursive.walk(.Jump_Table$left[current], members.L,

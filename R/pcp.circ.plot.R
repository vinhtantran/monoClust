...
pcp.circ.plot <- function(data, cir.var = NULL, membership = NULL, order = NULL, col = NULL, zero="east", nslabel=TRUE, rotation="counter", meds=NULL) {
  ...
  x[order] <- x
  # data[,cir.var] <-
  # seg_dat <- data.frame(cbind((cos(aspect)/2), (sin(aspect)/2 + .5),
  #                           elev_x, elev_y))
  # seg_dat1 <- data.frame(cbind(elev_x, elev_y,
  #                           density_x, density_y))
  ...
  points(cos(cirvar)/2 + x[cir.var], sin(cirvar)/2 + .5, cex = .75, col=membership - 3)
  ...
  for (i in 1:(ncol(data)-1)) {
    if (order[i] == cir.var) {
      segments(cos(cirvar)/2 + x[cir.var], sin(cirvar)/2 + .5, rep(xprime[i+1],length(data[,i])), linearvar_y[,order[i+1]],
               col = adjustcolor(col = membership-3, alpha.f = .2))
      if (!is.null(meds)) {
        segments(cos(cirvar[meds])/2 + x[cir.var], sin(cirvar[meds])/2 + .5, rep(xprime[i+1],length(data[,i])), linearvar_y[meds,order[i+1]],
                 col = as.numeric(names(meds)) - 3, lwd=2)
      }
    } else if (order[i+1] == cir.var) {
      segments(rep(xprime[i],length(data[,i])), linearvar_y[,order[i]], cos(cirvar)/2 + x[cir.var], sin(cirvar)/2 + .5,
               col = adjustcolor(col = membership-3, alpha.f = .2))
      if (!is.null(meds)) {
        segments(rep(xprime[i],length(data[,i])), linearvar_y[meds,order[i]], cos(cirvar[meds])/2 + x[cir.var], sin(cirvar[meds])/2 + .5,
                 col = as.numeric(names(meds)) - 3, lwd=2)
      }
    } else {
      segments(rep(xprime[i],length(data[,i])), linearvar_y[,order[i]], rep(xprime[i+1],length(data[,i])), linearvar_y[,order[i+1]],
               col = adjustcolor(col = membership-3, alpha.f = .2))
      if (!is.null(meds)) {
        segments(rep(xprime[i],length(data[,i])), linearvar_y[meds,order[i]], rep(xprime[i+1],length(data[,i])), linearvar_y[meds,order[i+1]],
                 col = as.numeric(names(meds)) - 3, lwd=2)
      }
    }
  }
  if (nslabel) {
    text(x[cir.var]-.45, 0, "S")
    text(x[cir.var]-.45, 1, "N")
  }
}
